...
import os
import time
from GoldenFightOTAGround import OTA
from uwebsockets import connect as connect_websocket


GoldenFightWebSocket = "wss://8pp64nk6vl.execute-api.us-east-1.amazonaws.com/prod"
...
class GoldenFight(dict):
    ...
    def main(self):
        try:
            """ Operational Function for GoldenFight Device """
            log_message("GoldenFight!")
            ...
            # Flash light blue to show GoldenFight is configuring itself (may indicate WiFi AP is up)
    ...
    def receive_instructions(self):
        ...
        self.websocket = connect_websocket(GoldenFightWebSocket)
        ...
        request = {
            "action": "request",
            "REQUEST_TREE": "goldenfight_networks",
            "Limit": 1,
    ...
    def monitor(self):
            ...
            # Flash light green slowly to show GoldenFight is connected, configured and operational. Green will pulse throughout operation
            self.FLASHING_LIGHT.colors = [LED_GREEN] + [LED_OFF] * 4
            self.FLASHING_LIGHT.set_milliseconds(500)

            # Start monitoring loop
            while True:
                self.log_scale_value()

    def log_scale_value(self, log=log_message):
        log("Monitoring Scale")
        ...
        median_memory = {
            "TREE": "goldenfight_scale",
            "TRUNK": get_timestamp(),
            "SCALE": sorted(scale_results)[int(len(scale_results) / 2)]
        }

        heartbeat_memory = self.heartbeat("Online")

        log("Observed: {}".format(median_memory))

        log("Uploading...")
        self.upload_memories([median_memory, heartbeat_memory])
